...
public class JdbcCoupon_bonde implements Coupon_bondeDao {

	Logger log = Logger.getLogger(JdbcCoupon_bonde.class);
	...
	@Value("${query.coupon_bondes.get.rate}")
	String getRate;
	...
	public Coupon_rate getRate(String date, String series) {
		...
		log.info("[JdbcCoupon_bonde][getRate]");
		
        log.info("[JdbcCoupon_bonde][getRate] date: " + date);
		
        log.info("[JdbcCoupon_bonde][getRate] series: " + series);

		log.info("[JdbcCoupon_bonde][getRate] exec query: query.get.parameter");
		...
		try (Connection con = dataSource.getConnection();
			...
			while (rs.next()) {
				...
				log.info("[JdbcCoupon_bonde][getRate] exec query Res : " + rs.getString(1));
				rate = rs.getString(1);
			...
			log.error("[JdbcCoupon_bonde][getRate] ERROR: Error al cargar la tabla cupon_bonde: " + e.getMessage());
		}
